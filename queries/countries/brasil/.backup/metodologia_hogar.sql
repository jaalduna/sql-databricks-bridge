-- table: metodologia_hogar
-- description: household methodology data with pnc and tivian calculations
-- source: brasil/queries.py::get_metodologia_hogar()
...
with pnc as (
    select
        ...
        count(distinct concat(convert(date,j.data_compra) , j.idcanal , j.idapp ,j.acceso_canal)) as viajes_x_hogar,
    ...
    from j_atoscompra_new (nolock)
    left join hato_cabecalho (nolock)
    ...
    where (
            ...
            or (j.periodo in (202408,202409) and j.flg_scanner in (1,2))
        )
        and j.periodo >= 202201
        and j.idpainel not in (14, 99)
        and h.flg_scanner >= 1 and h.flg_scanner <= 2
        and j.idproduto <> 999
    group by j.iddomicilio, j.periodo, j.seq
),
tivian as (
    select
        j.iddomicilio,
...
select
    ...
    cast(coalesce(p.iddomicilio, t.iddomicilio) as varchar) +
        cast(coalesce(p.seq, t.seq) as varchar) +
